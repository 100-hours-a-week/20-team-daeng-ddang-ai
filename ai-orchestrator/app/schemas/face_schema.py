...
# 얼굴 분석 결과 응답 스키마 (Flat Structure)
class FaceAnalyzeResponse(BaseModel):
    analysis_id: str = Field(..., description="분석 요청 식별자")
    predicted_emotion: str = Field(..., description="예측된 최종 감정 (happy, sad, angry, relaxed)")
    confidence: float = Field(..., description="최종 감정의 신뢰도 (0.0 ~ 1.0)")
    summary: str = Field(..., description="감정 분석 결과에 대한 한 줄 요약/나레이션")
    emotion_probabilities: Dict[str, float] = Field(..., description="각 감정별 확률 분포")
