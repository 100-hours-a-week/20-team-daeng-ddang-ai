# app/core/config.py
import os
from pathlib import Path

from dotenv import load_dotenv


# Load service-local .env regardless of the process working directory.
load_dotenv(Path(__file__).resolve().parents[2] / ".env")

# 디버그 모드 (True일 경우 상세 로그 및 사유 반환)
DEBUG = os.getenv("DEBUG", "false").lower() == "true"

FACE_MODE = os.getenv("FACE_MODE", "http").lower()
FACE_SERVICE_URL = os.getenv("FACE_SERVICE_URL", "http://localhost:8100").rstrip("/")
FACE_HTTP_TIMEOUT_SECONDS = float(os.getenv("FACE_HTTP_TIMEOUT_SECONDS", "20"))

FACE_JOB_MODE = os.getenv("FACE_JOB_MODE", "off").lower()

# Healthcare Service Config
HEALTHCARE_MODE = os.getenv("HEALTHCARE_MODE", "http").lower()
HEALTHCARE_SERVICE_URL = os.getenv("HEALTHCARE_SERVICE_URL", "http://localhost:8200").rstrip("/")
HEALTHCARE_HTTP_TIMEOUT_SECONDS = float(os.getenv("HEALTHCARE_HTTP_TIMEOUT_SECONDS", "240"))

# Chatbot Service Config
CHATBOT_MODE = os.getenv("CHATBOT_MODE", "http").lower()
CHATBOT_SERVICE_URL = os.getenv("CHATBOT_SERVICE_URL", "http://localhost:8300").rstrip("/")
CHATBOT_HTTP_TIMEOUT_SECONDS = float(os.getenv("CHATBOT_HTTP_TIMEOUT_SECONDS", "30"))

# Per-feature concurrency/backpressure controls
FACE_MAX_CONCURRENCY = int(os.getenv("FACE_MAX_CONCURRENCY", "2"))
FACE_QUEUE_WAIT_SECONDS = float(os.getenv("FACE_QUEUE_WAIT_SECONDS", "1.0"))

HEALTHCARE_MAX_CONCURRENCY = int(os.getenv("HEALTHCARE_MAX_CONCURRENCY", "2"))
HEALTHCARE_QUEUE_WAIT_SECONDS = float(os.getenv("HEALTHCARE_QUEUE_WAIT_SECONDS", "1.0"))

CHATBOT_MAX_CONCURRENCY = int(os.getenv("CHATBOT_MAX_CONCURRENCY", "1"))
CHATBOT_QUEUE_WAIT_SECONDS = float(os.getenv("CHATBOT_QUEUE_WAIT_SECONDS", "1.0"))
