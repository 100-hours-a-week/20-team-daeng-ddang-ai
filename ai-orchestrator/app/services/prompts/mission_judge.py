...
MISSION_REFERENCE_STORE_DATA = {
    ...
    "missions": {
        "SIT": {
            ...
            "mission_name": "앉아",
            "success_criteria": [
                "개(dog)의 엉덩이가 바닥에 닿아 있음",
                "개의 앞다리가 몸을 지탱하고 있음",
                "해당 자세가 최소 1초 이상 유지됨",
                "정확한 초 단위 측정이 어렵더라도, 앉은 상태가 명확히 지속되는 흐름이 보임"
            ],
            "failure_criteria": [
                "개(dog)의 엉덩이가 바닥에 닿지 않음",
                "자세가 1초 미만으로 유지됨",
        ...
        "DOWN": {
            ...
            "mission_name": "엎드려",
            "success_criteria": [
                "개(dog)의 가슴과 배가 모두 지면에 닿아 있고, 앞다리가 접힌 상태로 바닥에 놓인 자세가 확인됨",
                "카메라 각도상 가슴 전체가 명확히 보이지 않더라도, 몸통이 낮고 엎드린 자세가 분명함",
                "해당 자세가 최소 2초 이상 유지됨",
                "정확한 초 단위 측정이 어렵더라도, 엎드린 상태가 명확히 지속되는 흐름이 보임"
            ],
            "failure_criteria": [
                "개(dog)의 몸통이 지면에 닿지 않음",
                "개(dog)의 엉덩이만 내려가고 가슴이 들려 있음",
                "자세가 2초 미만으로 유지됨"
            ]
        },
        "PAW": {
            ...
            "mission_name": "손",
            "success_criteria": [
                "개(dog)의 앞발 중 한 발이 사람 손 위에 올라가 있고, 두 객체의 접촉이 명확하게 보임",
                "사람 손이 화면에 완전히 노출되지 않더라도, 접촉 순간이 분명함",
                "개가 의도적으로 앞발을 들어 손 위에 올리는 동작이 관찰됨"
            ],
            "failure_criteria": [
                "앞발만 들고 손과 접촉하지 않음",
                "접촉이 불명확하거나 매우 짧아 의도적 동작으로 보기 어려움"
            ]
        },
        "TURN": {
            "mission_type": "TURN",
            "mission_name": "돌아",
            "success_criteria": [
                "개(dog)의 몸통이 하나의 동작으로 연속적으로 회전함",
                "회전 각도가 270도 이상이거나, 제자리에서 한 바퀴에 가까운 회전이 관찰됨",
                "회전이 끊기지 않고 하나의 흐름으로 완성됨"
            ],
            "failure_criteria": [
                "회전 각도가 매우 작음",
                "우연한 방향 전환으로 보임",
        ...
        "JUMP": {
            ...
            "mission_name": "점프",
            "success_criteria": [
                "개(dog)의 앞발과 뒷발이 모두 지면에서 분리되거나, 뒷발은 지면에 닿은 상태로 두 앞발을 동시에 들어올림",
                "개(dog)의 몸 중심이 위로 상승하는 움직임이 관찰됨"
            ],
            "failure_criteria": [
                "개(dog)의 몸 중심이 위로 상승하는 움직임이 관찰되지 않음",
                "몸 중심 상승 없이 걷거나 서는 동작",
                "앞발만 들어올리고 뒷발은 지면에 유지되는 '차렷' 자세",
                "몸을 앞으로 쭉 뻗는 기지개 동작으로 점프로 보기 어려움"
            ]
        },
        "FREE": {
            "mission_type": "FREE",
            "mission_name": "자유 촬영",
            "success_criteria": [
                "영상 프레임 내에 개(dog)가 명확하게 등장함",
                "개(dog)의 신체 일부라도 식별 가능함"
            ],
            "failure_criteria": [
                "영상 내에 개(dog)가 보이지 않음",
                "사람, 고양이, 너구리, 다람쥐 등 개가 아닌 동물이나 생물만 등장함",
                "풍경, 바닥, 사물만 촬영됨"
            ]
        }
    }
...
[판정 원칙]
- 영상에 등장하는 주체가 반드시 "개(dog)"여야 합니다.
- 고양이, 사람, 너구리, 다람쥐 등 개가 아닌 경우 미션 내용과 무관하게 FAILURE로 판정하십시오.
- 개가 영상에 등장하지 않거나 확인할 수 없으면 FAILURE로 판정하십시오.
...
- 0.00      : 영상 내에 개가 등장하지 않거나 개가 아닌 동물/사람만 등장함
...
- 개(dog)가 아닌 동물이나 사람이 등장하는 경우, reason에 해당 피사체가 무엇인지 명시하십시오.
- 개가 보이지 않거나 동작이 확인 불가하면 그 사실을 reason에 명시하십시오.
